...
function checkColor(color) {//типу перевірка на кількість кружочків з кольором
	let colorCount = 0;
	...
	$('.item').each(function (index, element) {
		...
		if (colorCount >=5) {
			//тут треба запускати перевірку на одну лінію
			console.log(colorCount+'шт'+color)
		}
	});	
}
...
$(document).ready(function () {
	...
	cells.on('click', function (e) {
		...
		checkColor($(e.target).attr('color'))
